...
exports.NativeLanceManager = exports.VectorManager = void 0;
const nemotron_1 = require("../embedding/nemotron");
class VectorManager {
    sqliteVecStore;
    insertsSinceSave = 0;
    db;
    logger;
    constructor(db, sqliteVecStore, logger) {
        this.db = db;
    ...
    getCount() {
        try {
            const result = this.db.prepare('SELECT COUNT(*) as count FROM hnsw_meta').get();
            return result.count;
        }
        catch {
            return 0;
    ...
    async addEntry(entryId, text) {
        try {
            ...
            const entry = this.db.prepare('SELECT entry_type, tags FROM unified_entries WHERE id = ?').get(entryId);
            const entryType = entry?.entry_type || '';
            if (entryType === 'tool')
                return false;
            const embedding = await (0, nemotron_1.qwenEmbed)(text);
            if (!embedding || embedding.length !== nemotron_1.EMBED_DIM)
                return false;
            // Store in sqlite-vec
    ...
    async search(query, topK = 5, excludeTypes = ['tool']) {
        try {
            const embedding = await (0, nemotron_1.qwenEmbed)(query);
            if (!embedding || embedding.length !== nemotron_1.EMBED_DIM)
                return [];
            ...
            if (excludeTypes.length > 0) {
                ...
                for (const r of results) {
                    const entry = this.db.prepare('SELECT entry_type FROM unified_entries WHERE id = ?').get(r.entryId);
...
exports.NativeLanceManager = VectorManager;
//# sourceMappingURL=lance-manager.js.map